...
int main(void)
{
    ...
    if (shaprh)
    {
        ...
        //5.1000.9923.0 #0593948b
        //182573B3F
        VirtualProtectEx(shaprh, (LPVOID)(shapr3d_dll_base_addr + 0x2570000), 0x10000, PAGE_EXECUTE_READWRITE, &oldprotect);
        ptrAddress[0] = 0xe9;
        ptrAddress[1] = 0x7c;
        ptrAddress[2] = 0x02;
        ptrAddress[3] = 0x00;
        ...
        if (!WriteProcessMemory(shaprh, (LPVOID)(shapr3d_dll_base_addr + 0x2573B3F), (LPCVOID)ptrAddress, 6, &bytes_written))
        {
        ...
        if (!WriteProcessMemory(shaprh, (LPVOID)(shapr3d_dll_base_addr + 0x2573C19), (LPCVOID)ptrAddress, 2, &bytes_written))
        {
            printf("WriteProcessMemory failed! error code -> 0x%08x\n", GetLastError());
        }
        else
        ...
        VirtualProtectEx(shaprh, (LPVOID)(shapr3d_dll_base_addr + 0x2570000), 0x10000, oldprotect, &oldprotect);




        //1818BFD60
        VirtualProtectEx(shaprh, (LPVOID)(shapr3d_dll_base_addr + 0x18B0000), 0x10000, PAGE_EXECUTE_READWRITE, &oldprotect);
        ptrAddress[0] = 0x32;
        ptrAddress[1] = 0xc0;
        ptrAddress[2] = 0xfe;
        ptrAddress[3] = 0xc0;
        ...
        if (!WriteProcessMemory(shaprh, (LPVOID)(shapr3d_dll_base_addr + 0x18BFD60), (LPCVOID)ptrAddress, 5, &bytes_written))
        {
            printf("WriteProcessMemory failed! error code -> 0x%08x\n", GetLastError());
        }
        else
            printf("WriteProcessMemory success! -> %08llx bytes written.\n", bytes_written);
        VirtualProtectEx(shaprh, (LPVOID)(shapr3d_dll_base_addr + 0x18B0000), 0x10000, oldprotect, &oldprotect);
        ...
        if (!WriteProcessMemory(shaprh, (LPVOID)(shapr3d_dll_base_addr + 0x257DE90), (LPCVOID)ptrAddress, 3, &bytes_written))
        {
            printf("WriteProcessMemory failed! error code -> 0x%08x\n", GetLastError());
        }
        else
        ...
        if (!WriteProcessMemory(shaprh, (LPVOID)(shapr3d_dll_base_addr + 0x257DEC7), (LPCVOID)ptrAddress, 6, &bytes_written))
        {
            printf("WriteProcessMemory failed! error code -> 0x%08x\n", GetLastError());
        }
        else
