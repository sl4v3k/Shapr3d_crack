...
int main(void)
{
    ...
    if (shaprh)
    {
        ...
        DWORD oldprotect;
        VirtualProtectEx(shaprh, (LPVOID)(shapr3d_dll_base_addr + 0x1900000), 0x20000, PAGE_EXECUTE_READWRITE, &oldprotect);
        if (!WriteProcessMemory(shaprh, (LPVOID)(shapr3d_dll_base_addr + 0x190FFCE + 0x1000), (LPCVOID)ptrAddress, 5, &bytes_written))
        {
            printf("WriteProcessMemory failed! error code -> 0x%08x\n", GetLastError());
        }
        else
            printf("WriteProcessMemory success! -> %08llx bytes written.\n", bytes_written);
        VirtualProtectEx(shaprh, (LPVOID)(shapr3d_dll_base_addr + 0x1900000), 0x20000, oldprotect, &oldprotect);
    }
    else
