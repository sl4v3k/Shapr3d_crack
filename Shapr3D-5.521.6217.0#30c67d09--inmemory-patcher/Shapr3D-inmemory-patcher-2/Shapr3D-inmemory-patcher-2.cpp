...
int main(void)
{
    ...
    //5.381.5210.0 #f3afec7e
    //5.521.6217.0 #30c67d09
    if (shaprh)
    {
        ...
        DWORD oldprotect;
        VirtualProtectEx(shaprh, (LPVOID)(shapr3d_dll_base_addr + 0x1B30000), 0x20000, PAGE_EXECUTE_READWRITE, &oldprotect);
        if (!WriteProcessMemory(shaprh, (LPVOID)(shapr3d_dll_base_addr + 0x1B3FF43 + 0x1000), (LPCVOID)ptrAddress, 5, &bytes_written))
        {
            printf("WriteProcessMemory failed! error code -> 0x%08x\n", GetLastError());
        }
        else
            printf("WriteProcessMemory success! -> %08llx bytes written.\n", bytes_written);
        VirtualProtectEx(shaprh, (LPVOID)(shapr3d_dll_base_addr + 0x1B30000), 0x20000, oldprotect, &oldprotect);
    }
    else
